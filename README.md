...
5. 게임 시작 클릭시 게임화면으로 이동한다. 
6. 게임화면에는 플레이어,체력, 대쉬게이지, 스킬1, 스킬2, 소유무기, 소유골드, 미니맵이 표시되며 튜토리얼이 시작된다.
7. 조작은 wasd키가 이동키이며 점프는 스페이스바, 공격, 재장전, 상호작용, 스킬1, 스킬2, 인벤토리, 대쉬를 알려주고 마을에 도착한다.
8. 마을 오른쪽 끝부분으로 가면 던전으로가는 입구가 있다.
